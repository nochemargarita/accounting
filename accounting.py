...
def accounting(melon_cost, filename):
    """This function will iterate at each customer's number of order and their customer_payment.
    to check wherer customer have completed payment or have underpaid."""
    filename = open(filename)
    for line in filename:
        line = line.rstrip()
        split_str = line.split("|")

        expected = float(split_str[2]) * float(melon_cost)
        if float(expected) != float(split_str[3]):
            print split_str[1], "paid {:.2f}, expected {:.2f}".format(float(split_str[3]), float(expected))


accounting(1,"customer-orders.txt")
